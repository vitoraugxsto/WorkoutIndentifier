...
class RealtimeMovementValidatorML: # Renomeado para RealtimeMovementValidatorML
    ...
    def _extract_features_for_prediction(self, repetition_frames):
        ...
        """
        if not repetition_frames:
            return pd.DataFrame() # Retorna DataFrame vazio se não houver frames

        # Converte a lista de dicionários para DataFrame para facilitar o cálculo
        temp_df = pd.DataFrame(repetition_frames)

        # Se houver apenas 1 frame, std é NaN. Lidaremos com isso preenchendo com 0.
    ...
    def validate_video_realtime(self, video_path):
        ...
        while cap.isOpened():
            ...
            if results.pose_landmarks:
                ...
                # Adicionado 'desvio_5_frames' com valor placeholder, pois o modelo ML pode esperá-lo
                # No sistema real, você precisaria calcular este desvio em relação a uma referência.
                # Para simplificar a demonstração, vamos usar 0 ou um valor constante se não for calculada.
                current_frame_data = {
                    ...
                    'desvio_5_frames': 0.0 # Placeholder, ajuste se calcular em tempo real
                }
                ...
                if self.repetition_state == "EXTENDED":
                    ...
                    if (current_time - self.last_state_transition_time) > self.timeout_duration_seconds:
                        ...
                        if not features_for_prediction.empty:
                            # O modelo espera as características na mesma ordem de treinamento.
                            # Garanta que a ordem das colunas seja a mesma de X_train do script de treinamento.
                            # Para segurança, reordene as colunas se necessário:
                            # features_for_prediction = features_for_prediction[expected_feature_columns_order]
                            
                            prediction_encoded = self.ml_model.predict(features_for_prediction)[0]
                            prediction_proba = self.ml_model.predict_proba(features_for_prediction)[0] # [proba_incorreto, proba_correto]
                            
                            rep_is_correct = (prediction_encoded == 1) # 1 para correto, 0 para incorreto
                            ...
                            rep_is_correct = False
                            rep_feedback = f"Repetição {self.repetition_count}: Erro na extração de características para ML!"
                        # --- FIM DA SUBSTITUIÇÃO ---

                        repetition_completed = True 

                # Se uma repetição foi concluída (corretamente ou por timeout/erro)
                ...
                # --- Atualiza o texto de feedback na tela ---
            ...
            # Exibe o feedback e a contagem de repetições na tela
    ...
    def generate_summary_report(self):
        """Gera e imprime um relatório de resumo de todas as repetições."""
        ...
        if incorrect_reps > 0:
            ...
            for rep in self.all_repetition_results:
                if not rep['is_correct']:
                    reason = rep['reason']
                    # Limpa a mensagem para contar as razões primárias
                    reason_clean = re.sub(r'\(Confiança:.*?\)', '', reason) # Remove confiança ML
        ...
        if incorrect_reps == 0 and total_reps > 0:
            ...
            # Sugestões mais genéricas para o ML, pois ele não dá "razões" diretas como as regras
...
if __name__ == "__main__":
    # Caminho para o modelo ML treinado
    trained_model_file = 'rosca_quality_classifier_model.joblib' 
    
    # Caminhos para os CSVs de referência (usados para o _load_and_process_references, mas os limiares de validação são do ML)
    # São os mesmos usados para treinar o modelo.
    reference_csv_paths_for_init = ['rosca.csv', 'rosca2.csv'] 
    
    # Limiares de ângulo para a MÁQUINA DE ESTADOS (detecção de ciclo), não para a validação ML
    # Mantenha os que funcionaram para a detecção de repetições:
    min_angle_for_state_machine = 60    # Ângulo mínimo para a máquina de estados (fundo da rosca)
    max_angle_for_state_machine = 140   # Ângulo máximo para a máquina de estados (extensão do braço)
    ...
    # Substitua 'caminho/para/seu/video_a_analisar.mp4' pelo caminho REAL do vídeo que deseja analisar
    video_to_analyze_path = 'roscacor.mp4' 

    try:
        validator_ml.validate_video_realtime(video_to_analyze_path)
    except FileNotFoundError:
        print(f"Erro: O modelo '{trained_model_file}' não foi encontrado. Certifique-se de que o script de treinamento foi executado e o modelo foi salvo.")
